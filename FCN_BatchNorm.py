...
tf.flags.DEFINE_string('data_option', "fast", "data mode: normal/ fast")
...
def inference(image, keep_prob):
    ...
    with tf.variable_scope("FCN"):
        ...
        conv6_BN = tf.contrib.layers.batch_norm(inputs=conv6,is_training=is_Training)
        relu6 = tf.nn.relu(conv6_BN, name="relu6")
        ...
        conv7_BN = tf.contrib.layers.batch_norm(inputs=conv7, is_training=is_Training)
        relu7 = tf.nn.relu(conv7_BN, name="relu7")
        if FLAGS.debug:
        ...
        conv8_BN = tf.contrib.layers.batch_norm(inputs=conv8, is_training=is_Training)

        # now to upscale to actual image size
        deconv_shape1 = image_net["pool4"].get_shape()
        W_t1 = utils.weight_variable([4, 4, deconv_shape1[3].value, NUM_OF_CLASSES], name="W_t1")
        b_t1 = utils.bias_variable([deconv_shape1[3].value], name="b_t1")
        conv_t1 = utils.conv2d_transpose_strided(conv8_BN, W_t1, b_t1, output_shape=tf.shape(image_net["pool4"]))
        fuse_1 = tf.add(conv_t1, image_net["pool4"], name="fuse_1")
        fuse_1_BN = tf.contrib.layers.batch_norm(inputs=fuse_1, is_training=is_Training)

        deconv_shape2 = image_net["pool3"].get_shape()
        W_t2 = utils.weight_variable([4, 4, deconv_shape2[3].value, deconv_shape1[3].value], name="W_t2")
        b_t2 = utils.bias_variable([deconv_shape2[3].value], name="b_t2")
        conv_t2 = utils.conv2d_transpose_strided(fuse_1_BN, W_t2, b_t2, output_shape=tf.shape(image_net["pool3"]))
        fuse_2 = tf.add(conv_t2, image_net["pool3"], name="fuse_2")
        fuse_2_BN = tf.contrib.layers.batch_norm(inputs=fuse_2, is_training=is_Training)
        ...
        conv_t3 = utils.conv2d_transpose_strided(fuse_2_BN, W_t3, b_t3, output_shape=deconv_shape3, stride=8)
